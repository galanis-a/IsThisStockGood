...
from src.avData import avData
from src.pyEXData import pyEXData
...
def fetchDataForTickerSymbol(ticker):
  ...
  """
    ...
    main_data = data_fetcher.main_data
    margin_of_safety_price, sticker_price = _calculateMarginOfSafetyPrice(key_stats, pe_ratios, main_data)
    payback_time = _calculatePaybackTime(key_stats, main_data)
    template_values = {
        'ticker': ticker,
        'name': ticker,
        ...
        'current_price': main_data.current_price if main_data and main_data.current_price else 'null',
        'sticker_price': sticker_price if sticker_price else 'null',
        'payback_time': payback_time if payback_time else 'null',
        'average_volume': main_data.average_volume
    }
    return template_values


def _calculateMarginOfSafetyPrice(key_stats, pe_ratios, iex_data):
    if not key_stats or not pe_ratios or not iex_data:
        return None, None

    if not iex_data.five_year_growth_rate or not key_stats.equity_growth_rates:
        return None, None
    growth_rate = min(float(iex_data.five_year_growth_rate),
    ...
    if not iex_data or not iex_data.ttm_eps or not pe_ratios.pe_low or not pe_ratios.pe_high:
        return None, None
    margin_of_safety_price, sticker_price = \
        RuleOne.margin_of_safety_price(float(iex_data.ttm_eps), growth_rate,
                                       float(pe_ratios.pe_low), float(pe_ratios.pe_high))
    return margin_of_safety_price, sticker_price


def _calculatePaybackTime(key_stats, iex_data):
    if not key_stats or not iex_data or not iex_data:
        return None
    ...
    if not key_stats.last_year_net_income or not iex_data.market_cap:
        return None
    payback_time = RuleOne.payback_time(iex_data.market_cap, key_stats.last_year_net_income, growth_rate)
    return payback_time
...
class DataFetcher():
    ...
    def __init__(self, ):
        ...
        self.pe_ratios = None
        self.pyex_data_fetcher = None
        self.ev_data_fetcher = None
        self.main_data = None
        self.error = False
    ...
    def fetch_iex_data(self):
        self.pyex_data_fetcher = pyEXData(self.ticker_symbol, "pk_c9ca70e9882246f6b89530739a52d93c")
        self.main_data = self.pyex_data_fetcher.get_stock_data_from_pyex()

    def fetch_av_data(self):
        self.ev_data_fetcher = avData(self.ticker_symbol, "4OC8E0AGIQDVTI1P")
        self.main_data = self.ev_data_fetcher.get_stock_data_from_alpha_vantage()
